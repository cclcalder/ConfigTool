...
namespace WPF.ViewModels
{
    ...
    public abstract class WizardViewModelBase : ViewModelBase
    {
        ...
        private List<WizardPageViewModel> _pageList;

        public List<WizardPageViewModel> PageList
        {
            get { return _pageList ?? (_pageList = new List<WizardPageViewModel>(ConstructPageList())); }
            set
            {
                _pageList = value;
        ...
        public void NavigateToPageIndex(int value, Action callback)
        {
            if (_currentPageIndex != value)
            {
                ...
                int newIndex = value;
                WizardPageViewModel oldPage = PageList[oldIndex];
                WizardPageViewModel newPage = PageList[newIndex];
                ...
                else if (oldPage.Validate())// Warp factor 9 -> that way
                {
                    ...
                    if (oldPage.State == ToggleState.On ||  oldPage.Valid)
                    {
        ...
        {
            WizardPageViewModel wizardPageViewModel = PageList[_currentPageIndex];
            //if (!wizardPageViewModel.Visited)
            wizardPageViewModel.BeforeNavigateInTo();

            var templateIdx = ((PromotionWizardViewModel)wizardPageViewModel.PageView.DataContext).CurrentPromotion.Id;
            ThisWindow = ThisWindow ?? Application.Current.Windows.OfType<Window>().SingleOrDefault(x => (string)x.Tag == "promotion::" + templateIdx);
            ...
            if (ThisWindow == null)
            {
                if (callback != null)
                {
                    App.Navigator.NavigateTo(wizardPageViewModel.PageView, callback);
                }
                else
                {
                    App.Navigator.NavigateTo(wizardPageViewModel.PageView);
                }                
            }
            else
            {
                ThisWindow.Content = wizardPageViewModel.PageView;
        ...
        protected abstract IEnumerable<WizardPageViewModel> ConstructPageList();
        ...
        public int CurrentOffset(WizardPageViewModel page)
        {
        ...
        {
            foreach (WizardPageViewModel wizardPageViewModel in PageList.Skip(pageIndex + 1))
            {
                wizardPageViewModel.ForceReload = true;
            }
        }

        public void GoTo(WizardPageViewModel page)
        {
            int targetIndex = GetPageIndex(page);
            CurrentPageIndex = targetIndex;
        }

        protected int GetPageIndex(WizardPageViewModel page)
        {
            return PageList.IndexOf(page);
        }

        protected void ResetLaterPages(WizardPageViewModel pageViewModel)
        {
