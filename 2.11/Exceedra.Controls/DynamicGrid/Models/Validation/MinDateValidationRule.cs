...
namespace Exceedra.DynamicGrid.Models.Validation
{
    ...
    {
        public MinDateValidationRule(PropertyBase source, PropertyBase target) : base(source, target)
        {
        }
        ...
        public override string Validate()
        {
            ...
            // We only check if the sourceDate is after the targetDate.
            if (!isSourceDateParsed || !isTargetDateParsed)
                return string.Empty;

            if (sourceDate < targetDate)
                return "The " + Source.HeaderText + " is before the " + Target.HeaderText + ". Please select valid dates.";
