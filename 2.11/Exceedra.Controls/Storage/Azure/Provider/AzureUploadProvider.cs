...
namespace AzureProvider
{
    ...
    public partial class AzureUploadProvider : ICloudUploadProvider
    {
        ...
        public long FixedFileSize { get; set; }

        public StorageData UploadFileSettings { get; set; }
        ...
        public System.Threading.Tasks.Task<object> UploadFileAsync(string fileName, Stream fileStream, CloudUploadFileProgressChanged uploadProgressChanged, CancellationToken cancellationToken)
        {
            ...
            FixedFileSize = fileStream.Length;
            var part1 = System.IO.Path.GetExtension(fileName);
            

            FixedFilename = fileName.Replace(part1, "_" + DateTime.Now.Ticks.ToString() + part1);

            return System.Threading.Tasks.Task.Factory.StartNew<object>(() => UploadFileOnBlocks(FixedFilename, fileStream, uploadProgressChanged, cancellationToken), cancellationToken);
        }
        ...
        private StorageData GetUploadFileSettings()
        {
            StorageData settings = this.UploadFileSettings;
            if (settings == null)
            {
